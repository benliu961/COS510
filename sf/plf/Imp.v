...
Proof.
  intros. induction b; simpl;
    ...
    try (rewrite IHb1; rewrite IHb2; reflexivity). Qed.
...
Proof.
  split.
  - intros; induction H; simpl; subst; try reflexivity;
    try (apply aeval_iff_aevalR in H, H0; rewrite H; rewrite H0; reflexivity).
...
Proof.
  ...
  - inversion Heqloopdef. subst. simpl in *. discriminate.
  - inversion Heqloopdef. subst. simpl in *. apply IHcontra2. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - induction c; try constructor; 
    ...
    try apply IHc1; try apply IHc2; try assumption. discriminate.
  - intros. induction H; simpl; try reflexivity; apply andb_true_iff; split; assumption. Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct a; 
    ...
    try (simpl; destruct stack);
    try apply IHp1;
    try (destruct stack; apply IHp1). Qed.
...
Proof.
  intros st e.
  induction e; intros; simpl in *; try reflexivity;
  try (rewrite execute_app; rewrite execute_app; rewrite IHe1; rewrite IHe2; reflexivity).
  Qed.
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
  intros. inversion H0; subst; constructor; assumption. Qed.
...
Proof.
  intros. apply E_SeqContinue with (st' := st'); assumption. Qed.

Theorem seq_stops_on_break : forall c1 c2 st st',
  st =[ c1 ]=> st' / SBreak ->
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
  intros. apply E_SeqBreak. assumption. Qed.
(** [] *)
