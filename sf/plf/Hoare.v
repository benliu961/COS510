...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros. unfold valid_hoare_triple. intros. apply H. Qed.
(** [] *)
...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros. unfold valid_hoare_triple. intros. apply H in H1. destruct H1. Qed.
(** [] *)
...
Theorem hoare_asgn_wrong : exists a:aexp,
  ~ {{ True }} X := a {{ X = a }}.
Proof.
  simpl. unfold valid_hoare_triple.
  exists <{ X + 1}>. simpl.
  unfold not. intros.
  specialize H with (st := (X !-> 0 ; empty_st)) (st' := (X !-> 1 ; empty_st)).
  simpl in H.
  rewrite t_update_eq in H. 
  assert ( (X !-> 1; X !-> 0) = (X !-> 1)).
  { rewrite t_update_shadow. reflexivity. }
  assert ((X !-> 0) =[ X := X + 1 ]=> (X !-> 1; X !-> 0)).
  { apply E_Asgn. reflexivity. }
  rewrite H0 in H1. apply H in H1. 
  - inversion H1.
  - auto. Qed.
...
Proof.
  ...
  destruct H0.
  split.
  - rewrite t_update_shadow. rewrite <- H1. rewrite t_update_same. apply H0.
  - rewrite t_update_eq. rewrite t_update_shadow. 
    rewrite <- H1. rewrite t_update_same. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  unfold valid_hoare_triple.
  intros H.
  specialize H with (a := <{ Y + X }>) (n := 2).
  specialize H with (st := ( X !-> 2; Y !-> 0; empty_st)) (st' := (Y !-> 3; X !-> 3; empty_st)).
  simpl in H.
  assert ((X !-> 2; Y !-> 0) = (Y !-> 0; X !-> 2)).
    { apply t_update_permute. discriminate. }
  rewrite H0 in H.
  rewrite t_update_permute in H.
  + rewrite t_update_eq in H.
    assert ((X !-> 2; Y !-> 0) =[ X := 3; Y := Y + X ]=> (Y !-> 3; Y !-> 0; X !-> 3)).
    { apply E_Seq with (st' := (X !-> 3; X !-> 2; Y !-> 0; empty_st)).
      - apply E_Asgn. reflexivity.
      - rewrite t_update_shadow. rewrite t_update_permute.
        * apply E_Asgn. constructor.
        * discriminate. }
...
Proof.
  apply hoare_if; eapply hoare_consequence_pre;
    try apply hoare_asgn; try assertion_auto''. Qed.
(** [] *)
...
Proof.
  ...
  specialize H0 with (n := n). apply H0. Qed.   
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold "->>". unfold havoc_pre. intros.
    unfold assertion_sub. simpl. exists (aeval st X).
    rewrite t_update_shadow. rewrite t_update_same. assumption. Qed. 
...
Theorem assert_assume_differ : exists (P:Assertion) b (Q:Assertion),
  ...
  /\ ~ ({{P}} assert b {{Q}}).
Proof.
  exists (fun st => True). exists (BFalse). exists (fun st => True).
  split.
  - unfold valid_hoare_triple. intros. inversion H; subst. 
    exists st. split.
    + reflexivity.
    + assumption.
  - unfold valid_hoare_triple. intros. unfold not. intros.
    specialize H with (st := empty_st) (r := RError).
    assert (empty_st =[ assert BFalse ]=> RError).
...
(* FILL IN HERE *)
Theorem hoare_assert : forall (P Q : Assertion) (b : bexp),
  {{P /\ b}} skip {{Q}} ->
  P ->> (b) ->
  {{P}} assert b {{Q}}.
Proof.
  unfold valid_hoare_triple. intros.
  inversion H1; subst.
  - specialize H with (r := RNormal st) (st := st).
    apply H.
    + constructor.
    + split; auto.
  - unfold "->>" in H0.
    specialize H0 with st. apply H0 in H2.
    inversion H2. rewrite H4 in H5. discriminate. Qed.

Theorem hoare_assume : forall (P Q : Assertion) (b : bexp),
  {{P}} assume b {{P /\ b}}.
Proof.
  unfold valid_hoare_triple in *. intros.
  inversion H; subst. exists st. auto. Qed.
...
Example assert_assume_example:
  ...
  {{True}}.
Proof.
  apply hoare_seq with (X = 1)%assertion.
  - eapply hoare_seq with (X = 2)%assertion.
    + apply hoare_assert.
      * eapply hoare_consequence_post.
        { apply hoare_skip. }
        { eauto. }
      * unfold "->>". intros. simpl. eauto. 
    + unfold valid_hoare_triple. intros. inversion H; subst. 
      exists (X !-> 2; st). split.
  ...
  - unfold valid_hoare_triple. intros.
    ...
    + reflexivity.
    + simpl in *. apply eqb_eq. auto. Qed. 
